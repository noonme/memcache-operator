...
func (r *MemcachedReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			...
			klog.Info("memcached resource not found. Ignoring since object must be deleted", "req.NamespacedName", req.NamespacedName, "memcached", memcached)
	...
	// err = r.Get(ctx, types.NamespacedName{Namespace: memcached.Namespace, Name: "memcached-sample-7554dbfdf9-95gxb"}, foundPod)
	// if err != nil {
	// 	log.Error(err, "failed to get pod")
	// 	klog.Error(err, "failed to get pod in klog")
	// 	return ctrl.Result{}, err
	// } else {
	// 	klog.InfoS("foundPod is", "pod", foundPod)
	// }
	...
	if *found.Spec.Replicas != size {
		log.Info("log Deployment.Spec.Replicas != Memcached.size",
			"Deployment.Spec.Replicas", *found.Spec.Replicas, "Memcached.size", size)
		klog.InfoS("klog Deployment.Spec.Replicas != Memcached.size", "Deployment.Spec.Replicas", *found.Spec.Replicas, "Memcached.size", size)
	...
	// } else {
	// 	klog.InfoS("Deployment is exists", "Deployment ", found.ObjectMeta.Name, "The pod is", foundPod.ObjectMeta.Name)
	// }
...
// 根据memcached示例配置返回一个objcet deployment并创建一个deployment
